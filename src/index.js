import 'expose-loader?$!jquery';
import 'expose-loader?jQuery!jquery';
import 'expose-loader?ProgressBar!progressbar.js';
import './js/bootstrap';
import './js/slick';
import './js/validator';
import './js/ksc';
import './js/riot';

require("./brand/krowdspace-brand-assets.zip");
require("./brand/facebook-template.zip");
require("./brand/twitter-template.zip");
require("./brand/instagram-template.zip");
require("./brand/pinterest-template.zip");

require("./brand/krowdspace-press-release.pdf");
require("./brand/krowdspace-email-format.pdf");
require("./brand/kokoon.pdf");
require("./brand/oculus.pdf");
require("./brand/pebble.pdf");

require("./images/brand/krowdspace-alternative.svg");
require("./images/brand/krowdspace-alternative.png");
require("./images/brand/krowdspace-featured-alternative.svg");
require("./images/brand/krowdspace-featured-alternative.png");
require("./images/brand/krowdspace-featured.svg");
require("./images/brand/krowdspace-featured.png");
require("./images/brand/krowdspace-primary.svg");
require("./images/brand/krowdspace-primary.png");
require("./images/brand/krowdspace-primary-square.png");
require("./images/brand/krowdspace-reverse-alternative.svg");
require("./images/brand/krowdspace-reverse-alternative.png");
require("./images/brand/krowdspace-reverse-primary.svg");
require("./images/brand/krowdspace-reverse-primary.png");

require("./images/icons/krowdspace-favicon.jpg");
require("./images/icons/krowdspace-icon-57x57.jpg");
require("./images/icons/krowdspace-icon-72x72.jpg");
require("./images/icons/krowdspace-icon-114x114.jpg");
require("./images/icons/krowdspace-icon-144x144.jpg");

require("./images/giveaway/giveaway-01.png");
require("./images/giveaway/giveaway-02.jpg");
require("./images/giveaway/giveaway-03.jpg");
require("./images/giveaway/giveaway-04.jpg");
require("./images/giveaway/giveaway-05.jpg");
require("./images/giveaway/giveaway-06.jpg");
require("./images/giveaway/giveaway-07.jpg");
require("./images/giveaway/giveaway-08.jpg");
require("./images/giveaway/giveaway-09.png");

require("./images/about/about-01.png");
require("./images/about/about-02.png");
require("./images/about/about-03.png");
require("./images/about/about-04.png");
require("./images/about/about-05.png");
require("./images/about/about-06.png");
require("./images/about/about-07.png");
require("./images/about/andre.png");
require("./images/about/ben.png");
require("./images/about/david.png");
require("./images/about/mason.png");

require("./images/affiliates/krowdspace-design.png");
require("./images/affiliates/krowdspace-email-template.jpg");
require("./images/affiliates/krowdspace-press-release.jpg");
require("./images/affiliates/krowdspace-social.png");
require("./images/affiliates/launchboom.png");
require("./images/affiliates/nuuk.png");
require("./images/affiliates/spread-the-word.png");
require("./images/affiliates/sweeties.png");

require("./images/status/icon-approved.svg");
require("./images/status/icon-expired.svg");
require("./images/status/icon-pending.svg");

require("./images/indiegogo-full.svg");
require("./images/indiegogo-help-01.jpg");
require("./images/indiegogo-help-02.jpg");
require("./images/indiegogo.svg");
require("./images/kaisr.jpeg");
require("./images/pebble.png");
require("./images/kickstarter-full.svg");
require("./images/kickstarter.svg");
require("./images/krowdspace-header.png");
require("./images/krowdspace-logo-white.svg");
require("./images/krowdspace-logo.svg");

krowdspace.setDomain("https://api.krowdspace.com");
